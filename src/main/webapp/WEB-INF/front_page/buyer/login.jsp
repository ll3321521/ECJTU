...
<div class="w loc">
	<div class="h-title" id="logo">
		<div class="h-logo l"><img src="<%=request.getContextPath() %>/res/img/pic/logo-1.png"/></div>
		<div class="l" style="margin: 13px 10px;font-size: 20px;font-family: '微软雅黑';letter-spacing: 2px">欢迎登录</div>
	</div>
</div>
<div class="sign">
	<div class="l ad420x205"><a href="#" title="title"><img src="<%=request.getContextPath() %>/res/img/pic/ppp0.jpg" width="400" height="400"/></a></div>
	<div class="r">
		<h2 title="登录新巴巴运动网">登录新巴巴运动网</h2>
		<form id="jvForm" action="<%=request.getContextPath() %>/shop/login.shtml" method="post">
			...
			<li><label for="captcha">验证码：</label>
				...
				<img src="<%=request.getContextPath() %>/captcha" onclick="this.src='<%=request.getContextPath() %>/captcha?d='+new Date()" class="code" alt="换一张" /><a href="javascript:void(0);" onclick="$('.code').attr('src','<%=request.getContextPath() %>/captcha?d='+new Date())" title="换一张">换一张</a></li>
