...
function changeProvince(code) {
	var url = "${pageContext.request.contextPath}/buyer/city.shtml";
...
function changeCity(code) {
	var url = "${pageContext.request.contextPath}/buyer/town.shtml";
	var params = {"code" : code};
	$.post(url,params,function(data){
...
function addProfile(){
	...
	//复选框定位	$("input[name='']").attr("ckecked",true);
...
<div class="w mt ofc">
		...
		<div class="box bg_gray">
			<ul class="ul left_nav">
			<li><a href="<%=request.getContextPath() %>/buyer/profile.shtml" title="个人资料">个人资料</a></li>
			<li><a href="<%=request.getContextPath() %>/buyer/deliver_address.shtml" title="收货地址">收货地址</a></li>
			<li><a href="<%=request.getContextPath() %>/buyer/change_password.shtml" title="修改密码">修改密码</a></li>
			</ul>
		</div>
	</div>
		...
		<div class="confirm">
			...
			<div class="ofc">
				...
				<form id="jvForm">
						...
						<span class="word">
							<select name="province"  id="province" onchange="changeProvince(this.value)">
								...
								<c:forEach items="${provinces}" var="province">
									<option value="${province.code}" <c:if test="${province.code==buyer.province}">selected="selected"</c:if>>${province.name}</option>
								</c:forEach>
							</select>
							<select name="" id="city" onchange="changeCity(this.value)">
								<option value="" selected>城市</option>
								<c:forEach items="${cities}" var="city">
									<option value="${city.code}" <c:if test="${city.code==buyer.city}">selected="selected"</c:if>>${city.name}</option>
								</c:forEach>
							</select>
							<select name="" id="town">
								<option value="" selected>县/区</option>
								<c:forEach items="${towns}" var="town">
									<option value="${town.code}" <c:if test="${town.code==buyer.town}">selected="selected"</c:if>>${town.name}</option>
								</c:forEach>
							</select>
						</span>
					</li>
					...
					<li><label for="">&nbsp;</label><input type="submit" onclick="addProfile()" value="保存" class="hand btn66x23" /></li>
					</ul>
				</form>
			</div>
		</div>
