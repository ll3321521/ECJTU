...
//清空购物车
function clearCart() {
	window.location.href = "${pageContext.request.contextPath}/shop/clearCart.shtml";
}
//减购买数量
function subProductNumber(skuId) {
	//购买数量
	var number=$("#num"+skuId).val();
	number--;
	if(number==0){
		//购买数为0时删除
		delProduct(skuId)
	...
	//跟后台进行交互，改变价格
	window.location.href = '${pageContext.request.contextPath}/shop/cart.shtml?skuId=' + skuId + "&number=-1";
}
//加购买数量
function addProductNumber(skuId,buyLimit) {
	//购买数量
	var number=$("#num"+skuId).val();
	//购买限制
	if(number>=buyLimit){
		alert("该商品最多只能购买"+buyLimit+"件");
		return;
	}
	//跟后台进行交互，改变价格
	window.location.href = '${pageContext.request.contextPath}/shop/cart.shtml?skuId=' + skuId + "&number=" + 1 + "&buyLimit=" + buyLimit;
}
//从购物车中删除商品
function delProduct(skuId) {
	if (confirm("您确定删除吗？")) {
		window.location.href = '${pageContext.request.contextPath}/shop/delItem.shtml?skuId=' + skuId;
	}
}
//结算
function trueBuy(){
 	window.location.href = "${pageContext.request.contextPath}/buyer/trueBuy.shtml?returnUrl="+window.location.href;
}
</script>
</head>
<body>
		...
		<div class="ofc pb40">
			...
			<tbody>
				...
				<tr>
					...
					<td><a onclick="subProductNumber(${item.sku.id})" class="inb arr" title="减" href="javascript:void(0);">-</a>
					<input type="text" id="num${item.sku.id}" readonly="readonly" value="${item.number}" name="" size="1" class="txts">
					<a onclick="addProductNumber(${item.sku.id},${item.sku.skuUpperLimit})" class="inb arr" title="加" href="javascript:void(0);">+</a></td>
			...
			<div class="page">
				...
				<span class="r box_gray">
					<dl class="total">
						<dt>购物车金额小计：<cite>(共<var id="productAmount">${cart.number}</var>个商品)</cite></dt>
						<dd><em class="l">商品金额：</em>￥<var id="productPrice">${cart.price}</var>元</dd>
						<dd><em class="l">运费：</em>￥<var id="fee">${cart.fee}</var>元</dd>
...
<div class="w ofc case">
	<div class="confirm">
		...
		<div class="ofc pb40" style="text-align: center;height: 200px;margin-top: 80px">
       		 <a href="<%=request.getContextPath()%>" style="color: red;font-size: 30px;">去首页</a>挑选喜欢的商品
		</div>
	</div>
</div>
</c:if>
